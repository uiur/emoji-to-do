...
pub async fn create_issue(repo: &str, title: &str, body: &str) -> Result<Issue, Box<dyn std::error::Error>> {
  ...
  let resp = client.post(format!("https://api.github.com/repos/{}/issues", repo))
    ...
    .json(&params)
    .send()
    .await.map_err(|e| GithubClientError::ApiError)?;
  ...
  let issue = resp
    .json::<Issue>()
    .await.map_err(|e| GithubClientError::JsonError)?;
